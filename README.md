...
## Agent integration (Claude Code, Codex, Gemini, Cursor, OpenCode)

Skills and MCP servers live once under `.agents/` and get linked into per-tool config files:
...
Run the linker once after clone (and any time you edit `.agents/`):

```bash
npm run agents:link                       # all five tools
npm run agents:link -- --tool=claude      # just one
npm run agents:link -- --dry-run          # preview without writing
```

| Agent      | Skills                                                  | MCP                            |
| ---------- | ------------------------------------------------------- | ------------------------------ |
| Claude     | `.claude/skills/<n>` → `../../.agents/skills/<n>` (per-skill symlinks) | `.mcp.json` |
| Codex      | reads `.agents/skills/` natively                        | `.codex/config.toml`           |
| Gemini     | reads `.agents/skills/` natively                        | `.gemini/settings.json`        |
| Cursor     | reads `.agents/skills/` natively (v2.4+)                | `.cursor/mcp.json`             |
| OpenCode   | reads `.agents/skills/` natively                        | `opencode.json` (`mcp` block)  |

`AGENTS.md` is read natively by all five tools (Gemini via the `context.fileName` setting the linker writes). Four of five tools also read `.agents/skills/` directly — only Claude Code needs symlink bridges.

**Tool-only skills coexist:** the linker only manages symlinks to skills present in `.agents/skills/`. A real directory at `.claude/skills/<name>/` (or any other tool's skills dir) is left alone, so you can scope skills per-tool while sharing the rest.

**Safety:** JSON outputs merge with existing files (user keys preserved). Generated TOML/MDC files carry a `Generated by scripts/agents-link.ts` marker; the linker refuses to overwrite a non-marked file unless `--force` is set. Symlinks refuse to clobber a real file or directory.
...
- `/admin/users/[id]` — per-user detail with actions:
  ...
  Deliberately not exposed: a freeform "set password" — see [docs/DECISIONS.md](docs/DECISIONS.md).
...
See [docs/TUTORIAL.md](docs/TUTORIAL.md) for the full list with explanations. Highlights:
